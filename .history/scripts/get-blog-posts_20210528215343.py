...
def download_file(file_url, destination_folder):
    ...
    filename = hashlib.sha1(r.content).hexdigest()
    final_file_name = f'{filename}.{ext}'
    tmp_file_name = f'tmp.{ext}'
    tmp_file_path = os.path.join(destination_folder, final_file_name)
...
def process_block(block, text_prefix=''):
    ...
    for content in block.children:
        ...
        if content.type == 'header':
            ...
            text = text + text_prefix + f'![{content.title}]({content.link})'
        elif content.type == 'quote':
            text = text + text_prefix + '> {:s}'.format(content.title.replace("\n", "\n> "))
        else:

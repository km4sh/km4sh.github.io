...
NOTION_ROOT_PAGE_ID = os.getenv('NOTION_DATABASE_ID')
...
def to_markdown(id, ignore):
    base = client.get_block(url_or_id=id)
    title = base.title
    slug = slugify(title)
    text = ''
    metas = []
    if 'collection' in base.type:
        ...
        for row in base.collection.get_rows():
            ...
            metaText = '---\n' + '\n'.join(metas) + '\n---\n'
            text = metaText + text
    elif 'page' in base.type:
        print("-> Getting root page: {}".format(base.id))
...
if __name__ == "__main__":
    ...
    to_markdown(root_page_id, ignore=None)
